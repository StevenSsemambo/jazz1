...
const CI_STEPS=[
  ...
  {id:'mood',q:"How are you feeling right now? The honest version.",opts:['Good','Neutral','Struggling','Drained','Frustrated','Anxious']},
  {id:'oneThingMind',q:"What is the one thing on your mind most right now? The thing you are carrying.",opts:['Work/school','A relationship','Money','My health','A personal goal','Something I cannot name yet']},
  {id:'intention',q:"Last one: what is one intention you want to hold today?",opts:['Stay present','Be kind to myself','Get one hard thing done','Rest properly','Connect with someone','Let go of something']}
];
function startCheckIn(){
  ...
  const greeting=hr<12?'Morning, '+nm+'! Ready for your daily check-in?':hr<17?'Hey '+nm+'! Afternoon check-in time.':'Hey '+nm+'! Evening check-in.';
  const intro=greeting+' This takes about 2 minutes.';
...
function processCIStep(text){
  ...
  if(step.id==='sleep'){
    ...
    else if(text.toLowerCase().includes('rough'))logHealth('sleep',2);
    else if(text.toLowerCase().includes('okay'))logHealth('sleep',5);
    else if(text.toLowerCase().includes('decent'))logHealth('sleep',7);
    else if(text.toLowerCase().includes('great'))logHealth('sleep',9);
  }
  if(step.id==='mood'){
...
function finishCheckIn(){
  ...
  if(d.mood)summary+='You are feeling '+d.mood.replace(/[^a-zA-Z\s]/g,'').trim()+'. ';
  if(d.oneThingMind)summary+='You are carrying "'+d.oneThingMind+'" with you. ';
  if(d.intention)summary+='Intention: '+d.intention+'. ';
  if((P.checkInStreak||0)>1)summary+='\n\nThat is '+P.checkInStreak+' days in a row — that consistency matters more than you know.';
  else summary+='\n\nFirst check-in done. Come back tomorrow and we will start seeing real patterns.';
  if(d.mood&&(d.mood.includes('Struggling')||d.mood.includes('Anxious')))summary+='\n\nYou said you are struggling. I do not want to just move on. What is going on?';
  ...
  if(typeof checkGoalsDue==='function')checkGoalsDue();
  setQR([
    {l:"Let's talk about it",t:"I want to talk about what is on my mind"},
    {l:"I am good, just logging",t:"I just wanted to check in"},
    {l:"My goals",t:"Let us talk about my goals"},
    {l:"How are my patterns?",t:"Show me my stats and patterns"}
  ]);
  refreshStats();
}
...
function createGoal(title,why,category){
  ...
  P.goals.push(goal);
  saveP();
  if(typeof memAdd==='function')memAdd('Goal set: "'+title+'". Reason: "'+why+'"',['goal','goals',category],'excited','goals');
  return goal;
}
...
function checkGoalsDue(){
  ...
  if(overdue.length>0){
    ...
    setTimeout(()=>{
      const msgs=[
        'Before we go — I want to ask about your goal: "'+g.title+'". How is it going? Honestly.',
        'Quick goal check-in: "'+g.title+'". What is the latest? Any progress, even tiny?',
        'I have been thinking about your goal — "'+g.title+'". Where are you with it?'
      ];
      addMsg('b',rnd(msgs),'et-goal');
      histAdd('b',msgs[0],'neutral','goalCheck');
      setQR([
        {l:"Making progress!",t:"I have been making progress on my goal"},
        {l:"Stuck, need help",t:"I am stuck on my goal and need help"},
        {l:"I completed it!",t:"I completed my goal!"},
        {l:"I have paused it",t:"I have put my goal on hold for now"}
      ]);
...
// ── CRISIS RESOURCES ──────────────────────────────────────────────
const CRISIS_DB={
  UG:[{org:'Befrienders Uganda',num:'+256 800 212 121',note:'Free, confidential',hours:'24/7'},{org:'Mental Health Uganda',num:'+256 414 270 027',note:'Support',hours:'Mon-Fri 9am-5pm'}],
  KE:[{org:'Befrienders Kenya',num:'+254 722 178 177',hours:'24/7'},{org:'Kenya Red Cross',num:'1199',hours:'24/7'}],
  ZA:[{org:'SADAG Crisis Line',num:'0800 456 789',note:'Free',hours:'24/7'},{org:'Lifeline SA',num:'0861 322 322',hours:'24/7'}],
  NG:[{org:'MANI Nigeria',num:'+234 703 027 9279',hours:'9am-5pm WAT'}],
  GH:[{org:'Ghana Mental Health Authority',num:'+233 302 666 229',hours:'Office hours'}],
  TZ:[{org:'Muhimbili Hospital',num:'+255 22 215 0561',hours:'Office hours'}],
  ...
  GLOBAL:[{org:'Crisis Text Line',num:'Text HOME to 741741',hours:'24/7'},{org:'Befrienders Worldwide',num:'befrienders.org',note:'50+ countries',hours:'24/7'}]
};
function buildCrisisCard(){
  ...
  lines.forEach(l=>{
    const numClean=l.num.replace(/\s/g,'');
    const isDialable=/^[+\d]+$/.test(numClean);
    const link=isDialable?'href="tel:'+numClean+'"':'';
    html+='<div class="crisis-line"><div><div class="crisis-org">'+l.org+'</div><div class="crisis-country">'+(l.note||'')+' · '+l.hours+'</div></div><a class="crisis-num" '+link+'>'+l.num+'</a></div>';
  });
  html+='</div>';
  return html;
}
